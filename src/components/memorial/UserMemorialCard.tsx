...
const ReactionIcons: { [key in ReactionType]: React.ReactNode } = {
  liebe: (
    <svg width="28" height="28" viewBox="0 0 40 41" fill="none" xmlns="http://www.w3.org/2000/svg">
      <mask id="mask_liebe_small" style={{ maskType: 'alpha' }} maskUnits="userSpaceOnUse" x="0" y="0" width="40" height="41">
        <rect y="0.688477" width="40" height="40" fill="#D9D9D9"/>
      </mask>
      <g mask="url(#mask_liebe_small)">
  ...
  dankbarkeit: (
    <svg width="28" height="28" viewBox="0 0 41 41" fill="none" xmlns="http://www.w3.org/2000/svg">
      <mask id="mask_dankbarkeit_small" style={{ maskType: 'alpha' }} maskUnits="userSpaceOnUse" x="0" y="0" width="41" height="41">
        <rect x="0.75" y="0.688477" width="40" height="40" fill="#D9D9D9"/>
      </mask>
      <g mask="url(#mask_dankbarkeit_small)">
  ...
  blumen: (
    <svg width="28" height="28" viewBox="0 0 41 41" fill="none" xmlns="http://www.w3.org/2000/svg">
      <mask id="mask_blumen_small" style={{ maskType: 'alpha' }} maskUnits="userSpaceOnUse" x="0" y="0" width="41" height="41">
        <rect x="0.25" y="0.688477" width="40" height="40" fill="#D9D9D9"/>
      </mask>
      <g mask="url(#mask_blumen_small)">
...
interface UserMemorialCardProps {
  memorial: Memorial;
...
/**
 * UserMemorialCard Component
 *
 * Displays memorial info in the verwalten overview:
 * - Avatar (image, initials, or icon)
 * - Full name
 * - Birth/death dates with places
...
export function UserMemorialCard({
  ...
  const hasImage = memorial.avatar_type === 'image' && memorial.avatar_url;
  const hasInitials = memorial.avatar_type === 'initials';
    ...
    <div className={`relative overflow-visible ${className}`}>
      ...
      <div className="p-2 bg-bw border border-main rounded-lg flex max-w-xs min-w-[320px] flex-col relative z-10">
        ...
        <div className="flex flex-col gap-2 px-2 mb-6">
          ...
          {/* Dates */}
          <div className="flex flex-col gap-0.5 text-body-s text-secondary">
            {birthDate && (
              <div className="flex items-center gap-2">
                <span className="text-body-m">*</span>
                <span className="min-w-[5rem]">{birthDate}</span>
            ...
            {deathDate && (
              <div className="flex items-center gap-2">
                <span className="text-body-m">†</span>
                <span className="min-w-[5rem]">{deathDate}</span>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {/* Privacy Badge */}
            <Badge variant={memorial.privacy_level === 'public' ? 'public' : 'private'}>
              {memorial.privacy_level === 'public' ? 'Öffentlich' : 'Privat'}
            </Badge>
          </div>
        </div>

        {/* Reactions (if any) */}
        {totalReactions > 0 && reactions && (
          <div className="px-2 mb-2 flex flex-col gap-3 w-full">
            <div className="flex items-center gap-3">
              {REACTION_ORDER.map((type) => {
                const count = reactions[type];
                if (count === 0) return null;
                return (
                  <div key={type} className="flex items-center gap-1 text-interactive-info">
                    {ReactionIcons[type]}
                    <span className="text-body-s-semibold text-primary">{count}</span>
                  </div>
                );
              })}
            </div>
            ...
            <Link
              ...
              className="flex items-center justify-end gap-1 text-body-s text-link-default hover:underline"
            >
