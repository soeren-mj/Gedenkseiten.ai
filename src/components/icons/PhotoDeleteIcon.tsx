...
interface PhotoDeleteIconProps {
  size?: number;
  className?: string;
}

/**
 * PhotoDeleteIcon - Icon for deleting a photo
 * Used in avatar selection wizard step when photo is uploaded
 */
export default function PhotoDeleteIcon({ size = 16, className = '' }: PhotoDeleteIconProps) {
  return (
    <svg
      width={size}
      height={size}
      ...
      className={className}
    >
      <mask id="mask0_photo_delete" style={{ maskType: 'alpha' }} maskUnits="userSpaceOnUse" x="0" y="0" width="16" height="16">
        <rect width="16" height="16" fill="#D9D9D9" />
      </mask>
      <g mask="url(#mask0_photo_delete)">
        <path
